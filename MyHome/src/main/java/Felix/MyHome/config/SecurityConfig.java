package Felix.MyHome.config;

import Felix.MyHome.service.MyUserDetailsService;
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth->auth
                        ...
                        .anyRequest().permitAll().and())//можно только залогиненным
                .userDetailsService(myUserDetailsService)
                .headers(headers->headers.frameOptions().sameOrigin())
                .httpBasic(withDefaults()).logout().logoutUrl("/logout").logoutSuccessUrl("/").and()
                .build();
    }
